 ...
 * Returns undefined for a model the registry has never seen. */
export function modelInfo(
  modelId: string,
  presetId: ProviderId,
): ModelInfo | undefined {
  return (
    MODEL_REGISTRY.find((m) => m.id === modelId && m.presetId === presetId) ??
    MODEL_REGISTRY.find((m) => m.id === modelId && sameFamily(m.presetId, presetId))
  );
}
