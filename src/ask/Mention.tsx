...
// a saved query, a thread). One face, two homes: the composer's backdrop paints
// it behind the draft's glyphs while the textarea stays the source of truth,
// and the question echo wears it in the bubble, so the lift on send carries
// one face into the exchange. The pill is a marked span of selectable text,
// never a control (DESIGN rule 8's inverse): no role, no hover, no font of
// its own. The glyphs stay the surrounding text's, because a backdrop set in
// mono would drift off the glyphs it paints behind; the `@`, the quotes around
// a two-word name and the accent-soft fill are the data costume (WRITING,
// identifiers inside chrome). An `@word` that resolved to nothing gets no span
// and renders as the plain text it is (LESSONS 5: stale never refuses).
//
