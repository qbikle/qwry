...
- **A1 · Ask (v1)** — read-only question → SQL → answer panel. Gate: `EVAL.md` PR gate green; staging hybrid ≥ 22/23 on mid and large tiers; measured 2026-09-06 at `PROMPT_VERSION` v3 (W3): the six Pagila rows read green against the re-baselined file, staging Haiku 23/23 (met) and Sonnet 21/23 (one short, both losses passing on re-sample), EVAL.md §4.
