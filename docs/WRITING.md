...
the cheatsheet description). Deliberate HIG deviations, namely the lowercase
status register, data-state markers (`NULL`, `∅ empty`, `auto`), and in-place
mono shortcut hints, are a documented system, not drift; HIG has no register for
...
2. **The ellipsis is a contract.** `…` (the character, never `...`) on a
   ...
   means it acts immediately. Both directions are binding. Verified against
   Apple's shipping apps, 2026-07-24 (Safari Settings AutoFill "Edit…",
