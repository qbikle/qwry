...
(model configured, no thread): **qbot and the three starter suggestions**,
docked above the composer, and nothing else; no sentence about what Ask is or
does (DESIGN rule 11). qbot (B4) is the maintainer's lowercase q read as a
robot, an inline SVG standing in the free space above the starters: he wears
the pane's own `--accent` for a body (never a mascot palette: the pane speaks
for the data's origin, LESSONS 4), scales with the pane (72px at the 320
floor, 96 at 392, 112 from 440 up), and is not a control, so he takes no
pointer events, carries no text, no `title` and no handler, and is
`aria-hidden` (rule 8's inverse: no affordance at all). He is the amendment to
this paragraph's old "and nothing else": the sentence stays deleted, and the
one thing added above the starters says nothing. They are three of a pool of
twelve, generated by the connection's model once per schema shape (the
heuristic pool stands until it lands and wherever it cannot be asked),
advanced by three on every show, never a question already asked in any thread
of the connection, and the three crossfade when they change; no shuffle
...
qbot (B4, §1) is the one thing in the pane that LOOPS, and his loops are CSS
keyframes in `qbot.css`, not springs: a spring has a rest state to settle
into and a loop has none, so no preset is added and none is bent. Five of
them, every period a multiple of `--dur-slow` so the lint and the
reduced-motion kill both keep looking at tokens: a 4px bob over 13 beats
(3120 ms), the two legs swaying ±2° about their hips on the same period a
quarter beat behind it, the bulb glowing on a coprime 19 beats (4560 ms) so
the two never fall into a pattern, and the small eye highlight twinkling in
the waiting word's own uneven cadence (§2 item 2), its ten unlike spans baked
as keyframe percentages and its cycle length re-rolled by ±20% at every end,
so not even the 8 s cycle repeats. Everything rides transform or opacity;
nothing lays out. His entrance is `popIn` with the empty state, beside the
starters' `swapIn`, and his leave is SWAP (`swapIn`, the starters' own leave)
the instant the first question is sent, parked where he stood while the
thread takes the flow, so he is gone before the bubble's lift lands. The gaze
is a state and not a loop: the two eye marks drop 2px on `--dur-quick` /
`--ease-std`, the hover register, while the composer's textarea has focus.
Reduced motion is his rest pose, every loop off and the gaze a 2px jump; and
nothing animates while the pane is not on screen (`document.hidden`, closed,
or in Inspector mode: both holders stay mounted, so the loops PAUSE in place
and resume where they stopped, never from the top).
