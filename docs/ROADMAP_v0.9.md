...
> (parallel, file-disjoint) → gates (tsc, cargo check/clippy, unit tests,
...
3. Driver/edit-path changes: staging suite green:
   ...
   NEVER against prod.
   **Two staging DBs**: `bigdb` (112GB, real data, read-only /
   big-table perf tests, pre-existing scratch tests) and `scratchdb` (small, CREATE ok,
   ALL new DDL-creating fixtures: schemas/types/matviews/triggers, write-heavy
   round-trips, synthetic keyset benchmarks; env `QWRY_TEST_DB2=scratchdb`). scratchdb rule:
   fixtures live ONLY in a `qwry_test` schema, never public (it may hold real
   data), drop at teardown. scratchdb also serves as the second connection profile for
...
- [~] Perf budgets: keystroke path measured green (worst-case 4.2ms on 2.95MB,
      ...
      the punch list. Full staging regression at 0.9.0: 22+5 ✓ + keyset
...
- [x] Final adversarial audit: 1 S2 fixed (layered inline-editor Tab escape),
      ...
      "nothing in the diff threatens either invariant." Handed off.
