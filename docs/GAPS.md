...
- [ ] **Infinite scroll dups/drops rows without a unique sort** `[plan v0.4 keyset]` — `browser.ts:196` re-runs from row 0, no PK tiebreaker.
...
3. **O(doc) per keystroke ×3 in the editor** — updateListener `toString()` + connections.subscribe compare + tabs compare + whole-statement re-tokenize (`SqlEditor.tsx:133,152`, `context.ts:74`); 2MB paste = lag per key.
...
6. **Infinite scroll O(n²)** — re-streams from row 0 every page `[plan keyset]`; unbounded LIMIT growth vs 50k cap = expensive no-op loop at bottom — `browser.ts:196-204`.
7. **Inspector: unmemoized `JSON.parse` + pretty-stringify of the full cell on EVERY render; subscribes to whole statements/pending → re-parses per streamed batch** — `Inspector.tsx:49,146`. 8MB jsonb = 100s of ms per repaint. Raw-mode: full parse per keystroke (`Inspector.tsx:287`).
8. **JsonTree: no virtualization/child cap (50k-array = 50k components); registerRef effect missing dep array; search walks all nodes per keystroke** — `JsonTree.tsx:317,333,90`.
9. **SchemaTree not virtualized; re-renders every row per filter keystroke; 2 SVGs + tooltip string per row** — `SchemaTree.tsx:93`.
...
13. **FUNCS introspection pulls ~3k pg_catalog fns per connect/⌘R/DDL-sniff** `[plan]`; DDL-sniff regex false-positives re-introspect after ordinary queries (`schema.ts:80` `im` flags).
14. **⌘A+⌘C on 50k rows builds a >100MB string synchronously** — `Grid.tsx:245`.
15. **noAutocorrect MutationObserver runs selector work over every virtualized row mount during 60fps scroll** — `noAutocorrect.ts:29`.
16. **history LIKE full-scan per palette keystroke over unbounded table; single Mutex serializes all appdb** — `appdb.rs:192,41`.
17. Smaller: setSelectionValue O(N²) store writes `[plan]`; column-resize measure per mousemove; per-cell closures; smooth scrollIntoView `[plan]`; theme 40 setProperty `[plan]`; editor remount on theme change `[plan]`; springs replay + no reduced-motion `[plan]`; ~~tunnel 200ms poll quanta~~ (fixed 2026-07-06: 25ms→×2 backoff→200ms) + double 10s TLS retry stack (~30s worst 'connecting' with no abort).
...
- No ⌘/ shortcut cheat-sheet; no UI zoom (⌘+/-); window title never shows connection/db; breadcrumb not clickable.
- Modal a11y: no focus trap (Tab reaches hidden background buttons), no role=dialog, no focus restore.
- ContextMenu: no max-height scroll, no type-ahead, no checkbox items, second right-click doesn't re-open at new point.
- CloseGuard: plain Enter = destructive discard (inverts Mac convention — Postico requires ⌘⌫); DangerModal: no keyboard confirm, no typed-confirm tier for TRUNCATE/DROP.
- Empty states are dead ends (no CTA, no ⌘K hint).
