...
> Census of everything qwry is missing or getting wrong, produced by a 13-surface audit:
...
> daily-workflow enumeration). Raw per-surface reports with full detail live in
...
- [x] **Default-menu accelerator watch item**. RESOLVED 2026-07-03: app owns the native menu; Edit uses predefined items (native clipboard), customs emit app events; default File▸Close Window ⌘W gone. Original note: *(2026-07-02)*: an initial analysis suggested Tauri's default menu owns ⌘C/⌘A/⌘Z/⌘W/⌘V key equivalents; EMPIRICALLY ⌘C/⌘A/⌘Z/⌘W reach the webview on this setup and only ⌘V misbehaved (menu-item validation). Grid now has BOTH paths for paste (keydown fallback + paste event). Resolve the whole class when the native menu bar lands (v0.5 P0), own the menu, rebind Edit items to app events.
...
**Beat TablePlus at its own game** (a widely used incumbent; its known weaknesses = qwry's wedge):
